...
2. 如果传递的是 template 模板，模板会先经过编译器的解析，并最终根据不同的平台生成对应的代码，对应的是用 with 语句封装好的 render 函数；
...
1. vm. _c 方法是 template 内部编译成 render 函数时调用的方法；
2. vm. $createElement 是手写 render 函数时调用的方法；
...
1. 模板编译 render 函数，通常 template 模板通过编译生成的 render 函数都是 Vnode 类型，但是函数式组件返回的是一个数组，这时 Vue 的处理是将整个 children 拍平成一堆数组；
2. 用户定义的 render 函数：
  - 当 children 为文本节点时，这时 createTextVNode 创建一个文本节点 Vnode；
  - 当 children 中有 v-for 时会出现嵌套数组，这时的处理逻辑是遍历 children，对每个节点判断，如果依旧是数组则继续递归调用，直到类型为基础类型时，调用 createTextVNode 方法转化为 Vnode，
  - 这样递归，children 也变成了一个类型为 VNode 的数组；
...
_ update 的核心是 _ patch _ 方法：

1. 如果是服务端渲染，由于没有 DOM，_ patch _ 方法是一个空函数；
2. 在浏览器环境下，_patch _  是 patch 函数的引用；

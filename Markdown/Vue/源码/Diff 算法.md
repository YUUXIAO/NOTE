- 只对比父节点相同新旧子节点，时间复杂度只有 O（n）;
- 在 diff 比较的过程中，循环从两边向中间收拢；
- 新旧节点对比过程，不会对俩棵 Vnode 树进行修改，而是以比较的结果直接对真实 DOM 进行修改；
...
- 渲染函数：渲染函数是用来生成 Virtual Dom 的，Vue 使用模板来构建页面，在实现中会将模板编译成渲染函数；
- VNode 虚拟节点：节点描述对象，它描述了应该怎样去创建真实的 Dom 节点 ，通过 createElement 方法能将 Vnode 渲染成 Dom 节点；
- patch：它可以将 vnode 渲染成真实的 DOM，这个过程是对比新旧虚拟节点之间的不同，然后根据对比结果找出需要更新的节点进行更新；

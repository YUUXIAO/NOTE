...
> 当数据是对象时，调用walk 方法遍历数据的所有属性，并调用defineReactive 方法；
>
...
Object.defineProperty 方法在处理数组拦截上存在问题，只有在对数组重新赋值操作时才会被 setter 检测到，为了能检测到数组的变更操作，在传入的数据是一个数组时，Vue 会进行处理；
...
> computed 本质是一个惰性求值的观察者，实现利用了 Watcher 的嵌套收集；

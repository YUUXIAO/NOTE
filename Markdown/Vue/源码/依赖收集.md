...
> observe 方法主要用 data 作为参数去实例化一个 Observer 对象实例；
...
// 配置完数据
{
   // Observer类的实例，里面保存着Dep实例__ob__.dep => dep(uid:0)
    __ob__, 
    // 在闭包里存在dep(uid:1)
    a: 1,   
    // 在闭包里存在着dep(uid:2)，还有b.__ob__.dep => dep(uid:4)
    b: [2, 3, 4], 
    c: {
      	 // Observer类的实例，里面保存着Dep实例__ob__.dep => dep(uid:5)
        __ob__,
        // 在闭包里存在着dep(uid:6)
        d: 5 
    }
